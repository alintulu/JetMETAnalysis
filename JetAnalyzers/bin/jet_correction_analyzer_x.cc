...
int main(int argc, char **argv)
{
   ...
   for (unsigned int a = 0; a < algs.size(); a++)
   {
      ...
      if (!weightfilename.IsNull())
      {
         ...
         {
            cout << "Can't open " << weightfilename << endl;
         }
         cout << "Getting the weight histogram all_ ... " << flush;
         weightHist = (TH2D *)weightFile->Get((algs[a] + "/all_").c_str());
         if (weightHist == nullptr)
         {
            cout << "FAIL!" << endl
                 << "Histogram of weights named \"all_\" was not in file " << weightfilename << endl;
            return 0;
         }
         cout << "DONE" << endl;
      }
      ...
      {
         if (!doflavor && n == "refpdgid")
            continue;
         if (n == "rho_hlt" && 0 == chain->GetBranch("rho_hlt"))
            continue;
         if (n == "weight")
         {
            if (xsection > 0.0)
      ...
      for (unsigned int i = 0; i < levels.size(); i++)
      {
         stringstream sslvl;
         sslvl << "l" << levels[i];
         if (algs[a].find(sslvl.str()) != string::npos)
            exclude = true;
      }
      if (exclude)
      {
      ...
      for (unsigned int ievt = 0; ievt < nevt; ievt++)
      {
         ...
         {
            cout << "WARNING::Failed the pileup cut." << endl
         ...
         if (nrefmax > 0 && JRAEvt->nref > nrefmax)
            JRAEvt->nref = nrefmax;
...
*/
            ...
            if (ptgen < ptgenmin)
               continue;
            if (doflavor && abs(pdgid) != 123 && abs(JRAEvt->refpdgid->at(iref)) != abs(pdgid))
               continue;
            else if (doflavor && abs(pdgid) == 123 && (abs(JRAEvt->refpdgid->at(iref)) > 2 || abs(JRAEvt->refpdgid->at(iref)) == 0))
               continue;
            float eta = JRAEvt->jteta->at(iref);
            if (etamax > 0 && TMath::Abs(eta) > etamax)
               continue;
            float pt = JRAEvt->jtpt->at(iref);
            if (pt > 14000)
            {
               cout << "WARNING::pt>14000 GeV (pt = " << pt << " GeV)." << endl
                    << "Skipping this jet." << endl;
               continue;
            }
            float dr = JRAEvt->refdrjt->at(iref);
            if (drmax.size() > 0 && dr > drmax[a])
               continue;
            if (JetCorrector)
            {
               ...
               if (!L1FastJet)
                  JetCorrector->setNPV(JRAEvt->npv);
            }
            ...
            if (xsection > 0.0)
               weight = (xsection * luminosity) / nevt;
            if (useweight)
               weight = JRAEvt->weight;
            if (!(xsection > 0.0) && !useweight)
               weight = 1.0;
            if (weightHist != nullptr)
               weight *= weightHist->GetBinContent(weightHist->FindBin(ptgen, eta));
            if (!MCPUReWeighting.IsNull() && !DataPUReWeighting.IsNull())
            {
               double LumiWeight = LumiWeights_.weight(JRAEvt->tnpus->at(iIT));
               weight *= LumiWeight;
            }
            if (pThatReweight != -9999)
               weight *= pow(pthat / 15., pThatReweight);
            ...
            if (!reduceHistograms)
            {
               if (HigherDist->FindBin(scale * pt) < HigherDist->FindBin(ptgen))
                  HigherDist->Fill(scale * pt, weight);
               if (MiddleDist->FindBin(scale * pt) == MiddleDist->FindBin(ptgen))
                  MiddleDist->Fill(scale * pt, weight);
               if (LowerDist->FindBin(scale * pt) > LowerDist->FindBin(ptgen))
                  LowerDist->Fill(scale * pt, weight);
   ...
   cout << "jet_correction_analyzer_x" << endl
...
string get_flavor_name(int pdgid)
{
   ...
   if (abspdgid == 1 || abspdgid == 2)
      result = "qJ";
   else if (abspdgid == 123)
      result = "qJ";
   else if (abspdgid == 3)
      result = "sJ";
   else if (abspdgid == 4)
...
void makeResolutionHistogram(TH3F *RespVs_, TH1F *ResolutionVs_, TString slice, bool mpv, int slice_min, int slice_max)
{
   ...
   if (slice.CompareTo("y") == 0)
      bins = RespVs_->GetNbinsY();
   else if (slice.CompareTo("x") == 0)
      bins = RespVs_->GetNbinsX();
   else
   {
...
bool it_pileup(int itlow, int ithigh, vector<int> *npus, int iIT)
{
   if ((*npus)[iIT] >= itlow && (*npus)[iIT] <= ithigh)
      return true;
   return false;
}
                ...
                vector<int> *npus, int iIT)
{
   if (sumEOOT(npus, iIT) >= earlyootlow && sumEOOT(npus, iIT) <= earlyoothigh &&
       sumLOOT(npus, iIT) >= lateootlow && sumLOOT(npus, iIT) <= lateoothigh)
      return true;
   return false;
}
...
bool total_oot_pileup(int totalootlow, int totaloothigh, vector<int> *npus, int iIT)
{
   ...
   if (sumOOT >= totalootlow && sumOOT <= totaloothigh)
      return true;
   return false;
}
                ...
                int totallow, int totalhigh, vector<int> *npus, vector<int> *bxns)
{
   ...
   if (it_pileup(itlow, ithigh, npus, iIT) &&
       ...
       total_pileup(totallow, totalhigh, npus, iIT))
      return true;
   return false;
}
